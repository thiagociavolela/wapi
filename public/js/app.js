...
function renderProductDetail(product) {
  ...
  $('#product-detail').innerHTML = `<article class="product-card">${image ? `<img class="product-image" src="${escapeHtml(image)}" alt="${escapeHtml(product.name)}">` : '<div class="product-image product-image-empty">Sem imagem</div>'}<div class="product-card-body"><div class="product-card-title"><h3>${escapeHtml(product.name)}</h3><span class="product-status ${product.active ? 'active' : ''}">${product.active ? 'Ativo' : 'Inativo'}</span></div><div class="product-description">${safeProductDescription(product.descriptionHtml || product.description || product.shortDescription || 'Sem descrição cadastrada.')}</div><dl class="${discount ? '' : 'single'}"><div><dt>Valor do produto</dt><dd>${escapeHtml(productPrice(product.price))}</dd></div>${discount ? `<div><dt>Desconto</dt><dd>${escapeHtml(discount)}</dd></div><div class="product-final-price"><dt>Valor com desconto</dt><dd>${escapeHtml(discountedPrice)}</dd></div>` : ''}</dl><p class="product-installments">Em até 3x sem juros no cartão.</p><label class="product-description-option"><input id="send-product-description" type="checkbox" checked><span><strong>Enviar descrição completa</strong><small>Inclui os detalhes do produto na mensagem.</small></span></label><button id="send-product" class="primary" type="button"><svg viewBox="0 0 24 24"><path d="m22 2-7 20-4-9-9-4zM22 2 11 13"/></svg><span>Enviar produto</span></button></div></article>`;
}
...
$('#message-list').addEventListener('click', async event => {
  ...
  button.disabled = true; button.classList.add('retrying');
  try {
    await api(`/api/conversations/${state.active.id}/messages/${failed.id}/retry`, { method: 'POST' });
    state.pendingMessages = state.pendingMessages.filter(item => item.id !== failed.id);
    await Promise.all([loadMessages(), loadConversations()]); toast('Mensagem reenviada.');
  } catch (error) { toast(error.message); button.disabled = false; button.classList.remove('retrying'); }
});
